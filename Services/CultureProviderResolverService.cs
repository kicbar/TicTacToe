...
namespace TicTacToe.Services
{
    public class CultureProviderResolverService : RequestCultureProvider
    {
        ...
        public override async Task <ProviderCultureResult> DetermineProviderCultureResult(HttpContext httpContext)
        {
            ...
            else if (GetCultureFromQueryCookie(httpContext, out culture))
                return new ProviderCultureResult(culture, culture);
            else if (GetCultureFromQuerySession(httpContext, out culture))
                return new ProviderCultureResult(culture, culture);

            return await NullProviderCultureResult;
        }

        private bool GetCultureFromQuerySession(HttpContext httpContext, out string culture)
        {
            if (httpContext == null)
            {
        ...
        private bool GetCultureFromQueryCookie(HttpContext httpContext, out string culture)
        {
            if (httpContext == null)
            {
                throw new ArgumentNullException(nameof(httpContext));
        ...
        private string ParseCookieValue(string value)
        {
